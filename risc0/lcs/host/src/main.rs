use methods::{GUEST_RISC0_LCS_ELF, GUEST_RISC0_LCS_ID};
...
fn main() {
    ...
    // proof
    let it = Instant::now();
    let prove_info = prover.prove(env, GUEST_RISC0_LCS_ELF).unwrap();
    ...
    eprintln!("lcs({:?}, {:?}) = {res}", args.0, args.1);

    // verify
    let it = Instant::now();
    receipt.verify(GUEST_RISC0_LCS_ID).unwrap();
